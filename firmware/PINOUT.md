...
## Band — Raspberry Pi Pico W (original)
...
| GP6 | 9 | input, pull-up | BNO08x INT (H_INTN) | data-ready; optional but recommended |
| GP7 | 10 | output | BNO08x RST | firmware can reset the IMU; leave RST tied to 3V3 if you don't wire it |
...
| 3V3(OUT) | 36 | 3.3 V | SEN0240 ×2 VCC, BNO08x VIN, WS2812 +5V pad, motor +, green LED (via 1k) | 300 mA budget; total load ≈ 150 mA worst case |
...
Green 5 mm LED: VSYS → 1k → LED → GND (lit whenever the band is on or USB is plugged in; no GPIO).

Trunk to the forearm ring (6 wires, JST-PH 6 at the puck hub): 3V3, GND, EMG1, EMG2, SDA, SCL.
Cord to the hand (3 wires, JST-PH 3): TX, RX, GND (crossed at the hand: band TX → hand RX).

## Hand / claw — Raspberry Pi Pico 2 W
...
## Reserved / do not use
Pico W: GP23 (wireless power), GP24 (VBUS sense), GP25 (wireless SPI), GP29 (wireless / VSYS sense). Leave unconnected.
