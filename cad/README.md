# CAD (build123d via agentcad)

Replaces the OpenSCAD files in `band/` and `claw/` (kept as layout reference only; do not re-export them).

## Setup (done once, Windows)
```
uv venv --python 3.12 .venv
uv pip install --python .venv/Scripts/python.exe "agentcad[mcp]" build123d trimesh numpy
echo C:\Users\Tabor\Documents\GitHub\robokyle-factum\cad > .venv\Lib\site-packages\robokyle_cad.pth   # makes rk_parts importable
```
...
## Workflow
```
.venv\Scripts\agentcad run cad\fit_gauge.py --label gauge --export stl --no-view --no-diff
.venv\Scripts\python cad\publish.py gauge fit_gauge
```
